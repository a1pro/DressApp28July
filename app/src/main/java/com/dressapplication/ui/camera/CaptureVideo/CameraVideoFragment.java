...
public abstract class CameraVideoFragment extends BaseFragment {
    ...
    private static final String VIDEO_DIRECTORY_NAME = "DresssApp";
...
//            startRecordingVideo();
    ...
    public void reopenCamera(String id) throws CameraAccessException {
        if (mTextureView.isAvailable()) {
          //  openCamera(mTextureView.getWidth(), mTextureView.getHeight());
           configureTransform(mTextureView.getWidth(), mTextureView.getHeight());
            connectCamera();
                ...
                if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
                    if (ContextCompat.checkSelfPermission(getActivity(), Manifest.permission.CAMERA) ==
                        ...
                        if (shouldShowRequestPermissionRationale(Manifest.permission.CAMERA)) {
                            Toast.makeText(getActivity(), "Video app reqiured permissiontion to access camera", Toast.LENGTH_SHORT).show();
                        }
           ...
           // openCamera(mTextureView.getWidth(), mTextureView.getHeight());
            ...
            try {
                if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
                    if (ContextCompat.checkSelfPermission(getActivity(), Manifest.permission.CAMERA) ==
                            PackageManager.PERMISSION_GRANTED) {
                        cameraManager.openCamera("1", mStateCallback, mBackgroundHandler);
                    } else {
                        if (shouldShowRequestPermissionRationale(Manifest.permission.CAMERA)) {
                            Toast.makeText(getActivity(), "Video app reqiured permissitions to access camera", Toast.LENGTH_SHORT).show();
                        }
                        requestPermissions(new String[]{Manifest.permission.CAMERA, Manifest.permission.RECORD_AUDIO}, REQUEST_CAMERA_PERMISSION_RESULT);
                    }
                } else {
            ...
            if (cameraId.equals(CAMERA_BACK)) {
                if (isFlashSupported) {
                    if (isTorchOn) {
                      ...
                      //  flash_on_off.setImageResource(R.drawable.ic_flash_off_black_24dp);
                        isTorchOn = false;
                    } else {
                        mPreviewBuilder.set(CaptureRequest.FLASH_MODE, CaptureRequest.FLASH_MODE_TORCH);
                        mPreviewSession.setRepeatingRequest(mPreviewBuilder.build(), null, null);
                      //  flash_on_off.setImageResource(R.mipmap.flash_icon);
                        isTorchOn = true;
                    }
                }
            }
